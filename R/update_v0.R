...
PhenoIndex<-function(path,canum){
  ...
  for (i in c(1:length(videoname))){
    ...
    shellrun <- paste0("ffmpeg.exe -i ",name[i]," -ss 00:00:00  -r 12   -vframes 24 ",imgpath,"/TCL_",i,"_","%d.tif");     
    system(shellrun);
    
    for(j in c(1:24)){
      img_path<-paste0(imgpath,"/TCL_",i,"_",j,".tif");   #读取的视频帧图片的存放路径
      img<-readTIFF(img_path);
      if(greend<10){
        if(j<10){
          filename<-paste0(imgpath,"/R_pro/canum_day_00",greend,"_0",j,".tif");
          writeTIFF(img,filename);
          
        }else{
          filename<-paste0(imgpath,"/R_pro/canum_day_00",greend,"_",j,".tif");
          writeTIFF(img,filename);
        }
        
      }else if(greend < 100 && greend >= 10){
        if(j<10){
          filename<-paste0(imgpath,"/R_pro/canum_day_0",greend,"_0",j,".tif");
          writeTIFF(img,filename);
        }else{
          filename<-paste0(imgpath,"/R_pro/canum_day_0",greend,"_",j,".tif");
          writeTIFF(img,filename);
        }
        
      }else{
        if(j<10){
          filename<-paste0(imgpath,"/R_pro/canum_day_",greend,"_0",j,".tif");
          writeTIFF(img,filename);
        }else{
          filename<-paste0(imgpath,"/R_pro/canum_day_",greend,"_",j,".tif");
          writeTIFF(img,filename);
        }
      }
    }
  ...
  #处理图片
  setwd(imgpath,"/R_pro/");   #获取图片所在路径
  picturename = dir(pattern=".tif");
  for (k in c(1:length(picturename))){
    readfile_path<-paste0(imgpath,"/R_pro/",picturename[k]);
  ...
  logisticsg<-function(xx,yy,name){
    ...
    y<-fun(beta,x);
    #绘图
    jpeg(file="G://Pheno Result/re/",name,"g1.jpg");     #绘制图像的存储路径
    ...
    #绘图
    jpeg(file="G://Pheno Result/re/",name,"g2.jpg");
    ...
    dev.off();
    #保存
    write.xlsx(xx,"G://Pheno Result/re/",name,"g_xx.xlsx");
    write.xlsx(yy,"G://Pheno Result/re/",name,"g_yy.xlsx");
    write.xlsx(x,"G://Pheno Result/re/",name,"g_x.xlsx");
    write.xlsx(y,"G://Pheno Result/re/",name,"g_y.xlsx");
    write.xlsx(uk,"G://Pheno Result/re/",name,"g_uk.xlsx");
    write.xlsx(ky,"G://Pheno Result/re/",name,"g_ky.xlsx");
    write.xlsx(gromax,"G://Pheno Result/re/",name,"g_gromax.xlsx");
    #write.xlsx(grotime,"G://Pheno Result/re/",name,"g_grotime.xlsx");
    #write.xlsx(con,"G://Pheno Result/re/",name,"g_con.xlsx");
    #write.xlsx(I,"G://Pheno Result/re/",name,"g_I.xlsx");
  }
  ...
  logisticsd<-function(xx,yy,name){
    ...
    y<-fun(beta,x);
    #绘图
    jpeg(file="G://Pheno Result/re/",name,"d1.jpg");#绘制图像的存储路径
    plot(x,y,type = "l",col=6,xlab = "天数(DOY)",ylab = name,cex = 18,xaxt="n",lwd=2);
    axis(1,seq(100,300,50));
    points(xx,yy,pch=16,col=2);
    dev.off();
    ...
    #绘图
    jpeg(file="G://Pheno Result/re/",name,"d2.jpg");
    uk<-seq(xx[1],xx[length(xx)],length(x)-3);
    ...
    dev.off();
    #保存
    write.xlsx(xx,"G://Pheno Result/re/",name,"d_xx.xlsx");
    write.xlsx(yy,"G://Pheno Result/re/",name,"d_yy.xlsx");
    write.xlsx(x,"G://Pheno Result/re/",name,"d_x.xlsx");
    write.xlsx(y,"G://Pheno Result/re/",name,"d_y.xlsx");
    write.xlsx(uk,"G://Pheno Result/re/",name,"d_uk.xlsx");
    write.xlsx(ky,"G://Pheno Result/re/",name,"d_ky.xlsx");
    write.xlsx(gromax,"G://Pheno Result/re/",name,"d_gromax.xlsx");
    write.xlsx(grotime,"G://Pheno Result/re/",name,"d_grotime.xlsx");
    write.xlsx(con,"G://Pheno Result/re/",name,"d_con.xlsx");
  }
